...
function getLocalizationString(type, dataName, field) {
  if (
    localizationStrings[type] &&
    localizationStrings[type][dataName] &&
    localizationStrings[type][dataName][field]
  )
...
function init() {
  ...
  const fetchLocalizationPromises = localizationFiles.map((localization) =>
    fetch('data/Localization/' + lang + '/' + localization.filename)
      .then((res) => res.text())
      .then((text) => parselocalization(text, localization.type))
  );
  ...
  const fetchTemplatePromises = templateFiles.map((template) =>
    fetch('data/Templates/' + template.filename)
      .then((res) => res.text())
      .then((text) => parseTemplate(text, template.type))
  );
...
function parselocalization(text, localizationType) {
  ...
  lines.forEach((line) => {
    ...
    if (keySplit.length == 3) {
      ...
      localizationStrings[localizationType][keyId][keySplit[1]][keySplit[3]] =
        value;
    }
  });
}
...
function getTechBorderColor(techCategory) {
  if (techCategories[techCategory]) return techCategories[techCategory].color;
  return 'black';
}
...
function determineLevel(tech, validNodes) {
  ...
  if (tech.prereqs) {
    tech.prereqs.forEach((prereq) => {
      if (prereq === '' || !isValidNode(validNodes, prereq)) return;
