class TechSidebar extends React.Component {
  ...
  getReadableEffect(dataName) {
    const description = getLocalizationString(
      'effect',
      dataName,
      'description'
    );
    ...
    var replaceEffectTag = function (match) {
      switch (match) {
        ...
        case '{4}':
          return Math.abs(effectVal - 1.0).toLocaleString(locale, {
            style: 'percent',
          });
        ...
        case '{18}':
          return Math.abs(1.0 / effectVal - 1.0).toLocaleString(locale, {
            style: 'percent',
          });
    ...
    const effectTemplateString = description
      .replace(/^-/g, '')
      .replace(
        '<color=#FFFFFFFF><sprite name="mission_control"></color>',
        'Mission Control'
      )
  ...
  getReadableSummary() {
    ...
    if (summary.match(/<.*module>/)) {
      ...
      return React.createElement('p', null, summary);
    }
  }
  ...
  getObjectiveNames(objectiveName) {
    ...
    if (typeof objLocStrings === 'string') {
      ...
      Object.entries(objLocStrings).forEach((x) => {
        objStrings.push(
          x[1] + ' (' + getReadable('faction', x[0], 'displayName') + ')'
        );
      });
  ...
  findBlockingTechs(techToSearch) {
    return this.state.techTree.filter((tech) => {
      if (
        tech.prereqs &&
        tech.prereqs.find((prereq) => prereq === techToSearch.dataName)
      ) {
        return true;
  ...
  buildModuleDisplay(dataModule) {
    ...
    moduleDisplayElements.push(React.createElement('br'));

    if (icon) {
      moduleDisplayElements.push(
        React.createElement('img', { src: './icons/' + icon + '.png' })
      );
    }
    ...
    moduleDisplayElements.push(
      React.createElement('pre', null, JSON.stringify(dataModule.data, null, 2))
    );
  ...
  render() {
      ...
      MaterialUI.Button,
      {
        ...
        onClick: (event) => {
          let isolatedTree = this.getAncestorTechs(node)
            ...
            .concat(node);
          isolatedTree = [
            ...new Map(isolatedTree.map((v) => [v.dataName, v])).values(),
          ];
    ...
    const summaryLabel = React.createElement('h4', null, 'Summary');
    ...
    const treeCost =
      uniqueAncestorTree.reduce(
        (acc, curr) => acc + (curr.researchCost ? curr.researchCost : 0),
        0
    ...
    const costText = [
      React.createElement('h4', null, 'Cost: ', researchCost.toLocaleString()),
      React.createElement('h5', null, 'Total Tree Cost: ', treeCostString),
    ];
    ...
    if (node.isProject) {
      probabilities.push(
        React.createElement(
          ...
          (node.factionAvailableChance / 100).toLocaleString(locale, {
            style: 'percent',
          })
        )
      );
      probabilities.push(
        React.createElement(
          ...
          (node.initialUnlockChance / 100).toLocaleString(locale, {
            style: 'percent',
          })
        )
      );
      probabilities.push(
        React.createElement(
          ...
          (node.deltaUnlockChance / 100).toLocaleString(locale, {
            style: 'percent',
          })
        )
      );
      probabilities.push(
        React.createElement(
          ...
          (node.maxUnlockChance / 100).toLocaleString(locale, {
            style: 'percent',
          })
        )
      );
    ...
    if (node.isProject) {
      if (node.orgGranted && node.orgGranted !== '') {
        let displayName = getLocalizationString(
          'org',
          node.orgGranted,
          'displayName'
        );
    ...
    if (
          ...
          prereqElements.push(
            React.createElement(
              MaterialUI.Button,
              {
                ...
                variant: 'contained',
                className:
                  'prereqButton' + (tech.researchDone ? ' researchDone' : ''),
                size: 'small',
                title: tech.isProject ? 'Faction Project' : 'Global Research',
                'aria-label': tech
                  ? tech.displayName +
                    ' ' +
                    (tech.isProject ? 'Faction Project' : 'Global Research')
                  : '',
      ...
      prereqsText = React.createElement('h4', null, 'Required Research');
    ...
    if (blockingTechs.length > 0) {
          ...
          React.createElement(
            MaterialUI.Button,
            {
              ...
              'aria-label': blocked
                ? blocked.displayName +
                  ' ' +
                  (blocked.isProject ? 'Faction Project' : 'Global Research')
                : '',
      ...
      blockingText = React.createElement('h4', null, 'Unblocks Research');
    ...
    if (node.isProject) {
      if (
        ...
        node.requiredControlPoint.filter((region) => region !== '').length > 0
      ) {
    ...
    if (!node.isProject) {
      if (node.endGameTech) {
        endgame = React.createElement('h4', null, 'Endgame tech');
      }
    }
    ...
    if (node.isProject) {
      if (node.repeatable) {
        repeatable = React.createElement('h4', null, 'Repeatable');
      }
    }

    let claimsDescription, claimsList;
    if (node.isProject) {
      let claims = templateData['bilateral'].filter(
        (claim) =>
          claim.projectUnlockName == node.dataName &&
          claim.relationType == 'Claim'
      );
      if (claims.length > 0) {
        ...
        claimsDescription = React.createElement('h4', null, 'Claims');
        claimsList = React.createElement('ul', null, claimsElements);
      }
    }

    let adjacenciesDescription, adjacenciesList;
    if (node.isProject) {
      let adjacencies = templateData['bilateral'].filter(
        (adjaceny) =>
          adjaceny.projectUnlockName == node.dataName &&
          adjaceny.relationType == 'PhysicalAdjacency'
      );
      if (adjacencies.length > 0) {
        ...
        adjacenciesDescription = React.createElement('h4', null, 'Adjacencies');
        adjacenciesList = React.createElement('ul', null, adjacenciesElements);
      }
    }
    ...
    if (
      ...
      effectDescription = React.createElement('h4', null, 'Effects');
      effectList = React.createElement('ul', null, effectElements);
    }

    let orgMarketDescription, orgMarketList;
    if (!node.isProject) {
      let orgMarket = templateData['org'].filter(
        (org) => org.requiredTechName == node.dataName
      );
      if (orgMarket.length > 0) {
        ...
        orgMarketList = React.createElement('ul', null, orgMarketElements);
      }
    }

    let traitDescription, traitList;
    if (node.isProject) {
      let traits = templateData['trait'].filter(
        (aug) => aug.projectDataName == node.dataName
      );
      if (traits.length > 0) {
        ...
        traitList = React.createElement('ul', null, traitElements);
      }
    }
    ...
    if (quoteString) {
      quoteLabel = React.createElement('h4', null, 'Completion Quote');

      quoteText = React.createElement('p', {
        dangerouslySetInnerHTML: {
          __html: quoteString.replace(/\{(.*?)\}/g, this.getLeaderName),
        },
      });
    }
    ...
    if (node.isProject) {
      completionString = getLocalizationString(
        'project',
        node.dataName,
        'description'
      );
    ...
    if (completionString) {
      ...
      completionText = React.createElement('p', {
    ...
    if (node.isProject) {
      ...
      if (modules.length > 0) {
        ...
        moduleDescription = React.createElement('h4', null, 'Modules Unlocked');
        moduleList = React.createElement('p', null, moduleElements);
      }
    }
    ...
    return React.createElement(
      ...
      // Heading
      React.createElement('h2', null, node.displayName),
      costText,
      probabilities,
      factionAlways,
      factionReq,
